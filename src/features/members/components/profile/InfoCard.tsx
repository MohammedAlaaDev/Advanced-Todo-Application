import type { LanguageObject, LinksError, MemberObject, TempStackError } from "@/features/members/types";
import {
    Github,
    Linkedin,
    Globe,
...
const InfoCard = ({
    ...
    const stackValidation = useValidate<TempStackError>();
    const linksValidation = useValidate<LinksError>();

    const stackError = stackValidation.error?.tempStack;
    const linksError=linksValidation.error?.tempLinks;
    ...
    const stackFormRef = useErrorNavigation(stackValidation.shakeKey);
    const stackFocusRef = useRef<HTMLFormElement | null>(null);
    const linksFormRef = useErrorNavigation(linksValidation.shakeKey);
    const linksFocusRef = useRef<HTMLFormElement | null>(null);
    ...
    const handleUpdateStack = (e?: PreventableEvent) => {
        e?.preventDefault();

        const trimmedStack = localTempStack.map((skill) => skill.trim());
        ...
        stackValidation.validate(data, tempStackSchema, () => {
            const cleanedData = data.tempStack.filter((skill) => skill !== "");
    ...
    const handleUpdateLinks = (e?: PreventableEvent) => {
        e?.preventDefault();

        if (tempLinks) {
            const currentLinks = tempLinks.map((link) => link?.trim() || "");
            const data = {
    ...
    const openStackInputs = () => {
        ...
        setTimeout(() => {
            const firstInput = stackFocusRef.current?.querySelector("input");
            firstInput?.focus();
        }, 0);
    ...
    const openLinksInputs = () => {
        ...
        setTimeout(() => {
            const firstInput = linksFocusRef.current?.querySelector("input");
            firstInput?.focus();
        }, 0);
                    ...
                    </div>
                    {
                        stackEditMode ?
                            <div className="flex gap-2">
                                ...
                                <Button className="size-6" onClick={handleUpdateStack}>
                        ...
                        <form
                            ...
                            {localTempStack.map((skill, idx) => (
                                <div key={idx} className="flex flex-col items-start">
                                    ...
                                    <InputError className='text-xs!' key={stackValidation.shakeKey} message={stackError?.[idx]?._errors[0]} />
                                </div>
                            ))}
                            {
                    ...
                    <h3 className="text-sm font-semibold text-gray-400 dark:text-gray-500 uppercase">Social Media</h3>
                    {
                        linksEditMode ?
                            <div className="flex gap-2">
                                ...
                                <Button className="size-6" onClick={handleUpdateLinks}>
                                    <Check className="text-white" />
                                </Button>
                            </div>
                            :
                ...
                </div>
                {
                    linksEditMode ?
                        <>
                            <form ref={(node) => {
                                linksFormRef(node);
                                linksFocusRef.current = node;
                            }} className='flex flex-col gap-2' onSubmit={handleUpdateLinks}>
                                <Button type='submit' className='hidden'>
                                </Button>
                                {
                                    tempLinks?.map((link, idx) => (
                                        <div key={idx}>
                                            ...
                                            <InputError key={linksValidation.shakeKey} message={linksError?.[idx]?._errors[0]} />
                                        </div>
                                    ))
                                }
                            </form>
