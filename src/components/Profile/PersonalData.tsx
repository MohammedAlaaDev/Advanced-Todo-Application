...
import { editDescription, editEmail, editPhone } from "@/features/members/membersSlice";
import { useDispatch } from "react-redux";
import { useRef, type SubmitEvent, type Dispatch, type SetStateAction } from "react";
...
const PersonalData = ({
    ...
    const descriptionValidation = useValidate();
    const emailValidation = useValidate();
    const phoneValidation = useValidate();

    const handleDescriptionEdit = (e?: SubmitEvent<HTMLFormElement>) => {
    ...
    const handleEmailEdit = (e?: SubmitEvent<HTMLFormElement>) => {
        if (e) e.preventDefault();
        const data = {
    ...
    const handlePhoneEdit = (e?: SubmitEvent<HTMLFormElement>) => {
        if (e) e.preventDefault();
        const data = {
    ...
    const openEmailInput = () => {
        resetEditModes();
        emailValidation.setError(undefined);
    ...
    const openPhoneInput = () => {
        resetEditModes();
        phoneValidation.setError(undefined);
    ...
    const openDescriptionInput = () => {
        resetEditModes();
        descriptionValidation.setError(undefined);
                    ...
                    </div>
                    {
                        emailEditMode ?
                            <form
                                ...
                                <div>
                                    <InputError key={emailValidation.shakeKey} message={emailValidation.error?.email?._errors[0]} className="mb-2" />
                        ...
                        <span className="font-medium">phone</span>
                        {
                            phoneEditMode ?
                                <form
                                    ...
                                    <div>
                                        <InputError key={phoneValidation.shakeKey} message={phoneValidation.error?.phone?._errors[0]} className="mb-2" />
                                        <div className="flex gap-2 items-center">
                                            <Button className="w-6 h-6 text-white" onClick={() => {
                                                setPhoneEditMode(false)
                                            }}>
                    ...
                    <div className="relative group flex flex-wrap gap-2 items-end">
                        {
                            descriptionEditMode ?
                                <>
                                    ...
                                    />
                                    <div>
                                        <InputError key={descriptionValidation.shakeKey} message={descriptionValidation.error?.text?._errors[0]} className="mb-2" />
